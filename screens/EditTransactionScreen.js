...
const EditTransactionScreen = ({ route, navigation }) => {
  ...
  const { categories, selectedCurrency, updateTransaction, isPremium, addCategory } = useBudgetStore();
  ...
  const [isRecurring, setIsRecurring] = useState(transaction.is_recurring === 1);
  const [frequency, setFrequency] = useState(transaction.frequency || 'monthly');
  const [excludeFromLimits, setExcludeFromLimits] = useState(transaction.exclude_from_limits === 1);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      Alert.alert('Error', 'Failed to update transaction. Please check your input and try again.');
