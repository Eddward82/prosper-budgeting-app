...
/**
 * Shared transaction form used by both the Add and Edit screens.
 *
 * Props:
 *  - navigation:     react-navigation prop (used for goBack / upgrade nav)
 *  - initialValues:  { amount, type, categoryId, date, tags, isRecurring, frequency, excludeFromLimits }
 *  - submitLabel:    text for the submit button (e.g. "Add Transaction")
 *  - onSubmit:       async (values) => void — performs the DB write
 *  - getSuccessMessage: (type) => string — alert shown after a successful submit
 *  - errorMessage:   string shown when onSubmit throws
 */
const TransactionForm = ({
  ...
  const { categories, selectedCurrency, isPremium, addCategory } = useBudgetStore();
  ...
  const [excludeFromLimits, setExcludeFromLimits] = useState(initialValues.excludeFromLimits);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await onSubmit({
        ...
        nextRunDate,
        excludeFromLimits: excludeFromLimits ? 1 : 0
      });

      Alert.alert('Success', getSuccessMessage(type));
      navigation.goBack();
    } catch (error) {
